    ...
    for i in range(1, 6):
        ...
        if player_name_key in members_dict and player_link_key in members_dict:
            ...
            player_id = player_id_match.group(
                1) if player_id_match else "Unknown"

            unique_players[player_name] = (
                player_id,
                player_link,
            )  # чтобы чтоб без дупликатов

unique_players_df = pd.DataFrame(
    unique_players.items(), columns=["Player Name", "Player Info"]
)
unique_players_df[["Player ID", "Link"]] = pd.DataFrame(
    unique_players_df["Player Info"].tolist(), index=unique_players_df.index
)
