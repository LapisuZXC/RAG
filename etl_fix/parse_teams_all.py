    ...
    try:
        ...
        for index, parent_element in enumerate(parent_elements, start=1):
            ...
            try:
                ...
                hltv_points = child_element_hltv_points.text
                # -----------------парсим игроков-----------------------
                ...
                data["HLTV_points"].append(hltv_points)
    ...
    for ind, cur_url in enumerate(urls):
        ...
        with driver_context_manager() as driver_manager:
            ...
            if isValid:
                ...
                urls = urls[ind::7][1:] # начинаем искать каждую неделю и пропускаем известную неделю
                break
